...
export const BarChart: React.FC<BarChartProps> = ({ title, data, dataKeyX, dataKeyY, barKey, layout, barColor = 'hsl(var(--primary))', isCurrency = false, isPercentage = false, className, delay = 0 }) => {
  ...
  const chartContent = (
    <Card className={cn("h-96 flex flex-col transition-all duration-300 hover:shadow-xl hover:shadow-primary/30 hover:scale-[1.01] hover:ring-2 hover:ring-primary/50", className)}>
      <CardHeader>
